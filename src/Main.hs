...
import           Data.Maybe
import           Unbound.LocallyNameless          hiding (empty, join, tree2listBy)
...
newVar = fresh (s2n "_")
assign v t = modify (Map.insert v t)
deref v = gets (Map.lookup v)
...
eq :: Tm -> Tm -> K ()
eq t1 t2 = join $ e <$> lift(reduce'<$>expand t1) <*> lift(reduce'<$>expand t2)
  where
    e (Var x) (Var y) | x == y  = ok
    e (Var x) t  = lift $ assign x (reduce' t)
    e t (Var x)  = lift $ assign x (reduce' t)
...
(/==) :: Tm -> Tm -> Goal
a /== b = guard =<< (/=) <$> lift(expand a) <*> lift(expand b)
